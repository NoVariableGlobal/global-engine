...
void OrientateToMouseIC::handleInput(const SDL_Event& _event)
{
	...
	TransformComponent* fatherTransform = dynamic_cast<TransformComponent*>(father->getComponent("TransformComponent"));
	...
	if (xMouse > xWinSize) xMouse = xWinSize;
	if (yMouse > yWinSize) yMouse = yWinSize;

	Ogre::Vector3 newOri = Ogre::Vector3(abs(fatherPos.x - xMouse), abs(fatherPos.y - yMouse), fatherPos.z);
	fatherTransform->setOrientation(newOri);
	//std::cout << fatherTransform->getOrientation().x << " " << fatherTransform->getOrientation().y << "\n";
}
